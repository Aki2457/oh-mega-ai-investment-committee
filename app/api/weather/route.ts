...
const types = ["gear", "stock_allocation", "halt", "buy"];
const modes: Mode[] = ["Balanced", "Attach", "Lockdown"];
...
export async function POST(request: Request) {
  ...
  if (body.type === "stock_allocation" && (!Number.isFinite(Number(body.value)) || Number(body.value) < 0 || Number(body.value) > 75)) return Response.json({ error: "Stock allocation must be between 0 and 75, preserving at least 25% cash" }, { status: 400 });
  if (body.type === "buy" && !body.ticker?.trim()) return Response.json({ error: "Ticker is required" }, { status: 400 });
