...
export function ResearchChat() {
  ...
  function CommitteeView() {
    ...
    const needsRun = !latestDecision;
    const taskTitle = !setupReady ? "System setup needs attention" : needsApproval ? `Review ${pendingCount} stock candidate${pendingCount === 1 ? "" : "s"}` : needsRun ? "Run the first weekly committee" : activeMods.length ? "Review the active portfolio instruction" : "Nothing required from you";
    const taskDetail = !setupReady ? "Open the setup guide and connect the missing service." : needsApproval ? "This is the only manual approval required before the next allocation." : needsRun ? "The committee will research, review Risk, and create the paper allocation." : activeMods.length ? "Confirm the temporary instruction still makes sense." : "The weekly committee is automatic. You can check the position or ask a question.";
    const taskAction = !setupReady ? () => setView("workflow") : needsApproval ? () => setView("universe") : needsRun ? runNow : activeMods.length ? () => setView("weather") : () => setView("portfolio");
    return <div className="simple-home">
      <section className={`simple-task ${!needsApproval && !needsRun && setupReady && !activeMods.length ? "task-clear" : ""}`}><div><p>YOUR WORK</p><h2>{taskTitle}</h2><span>{taskDetail}</span></div><button onClick={taskAction} disabled={running}>{running ? "Committee working" : !needsApproval && !needsRun && setupReady && !activeMods.length ? "View portfolio" : "Do this now"} →</button></section>
      <section className="simple-position"><div className="simple-position-head"><div><p>CURRENT FUND POSITION</p><h2>{weatherMode}</h2><span>{String(latestFinal.rationale ?? latestDecision?.rationale ?? "Awaiting the first committee decision.")}</span></div><ModeBadge mode={weatherMode} /></div><div className="simple-numbers"><button onClick={() => setView("portfolio")}><span>Stocks</span><strong>{displayStockPct.toFixed(0)}%</strong></button><button onClick={() => setView("portfolio")}><span>Cash</span><strong>{displayCashPct.toFixed(0)}%</strong></button><button onClick={() => setView("portfolio")}><span>Paper NAV</span><strong>${currentNav.toFixed(2)}</strong></button><button onClick={() => setView("risk")}><span>Risk view</span><strong>{String(latestDecision?.riskOpinion ?? "Pending")}</strong></button></div><div className="simple-position-actions"><button onClick={() => setView("portfolio")}>Open portfolio</button><button onClick={() => setView("weather")}>See controls</button></div></section>
      <section className="panel chat-card simple-chat" id="agent-chat"><div className="panel-head"><div><p className="eyebrow">ONE PLACE TO ASK</p><h3>Ask the committee</h3><span>Think Standard combines Research, Risk, and CIO judgment in one answer.</span></div></div><div className="chat-stream">{messages.map((message) => <div className={`chat-message ${message.role}`} key={message.id}><span>{message.role === "assistant" ? "COMMITTEE" : "YOU"}</span><MarkdownMessage text={message.text || "Thinking..."} />{message.model && <small>{message.model}</small>}</div>)}</div><form className="chat-composer" onSubmit={sendMessage}><input value={input} onChange={(event) => setInput(event.target.value)} placeholder="Ask about the current position" /><button disabled={chatting || !setupReady}>{chatting ? "Working" : "Ask"}</button></form></section>
      <details className="explore-details"><summary>Explore details and the JRPG house</summary><div><ActionCenter /><LivingRoom /><WeatherStrip /><section className="decision-cockpit"><div className="decision-copy"><div className="decision-label"><p className="eyebrow">LATEST DECISION</p><ModeBadge mode={displayMode} /></div><h2>{displayMode} mode</h2><p>{String(latestFinal.rationale ?? "Detailed committee evidence appears after the first run.")}</p></div><AllocationDonut stockPct={displayStockPct} cashPct={displayCashPct} label={displayMode} /><div className="probability-panel"><ProbabilityBar label="US technology" value={Number(latestFinal.usUpProbability ?? 0) * 100} detail="Next-week probability" /><ProbabilityBar label="China / HK technology" value={Number(latestFinal.chinaUpProbability ?? 0) * 100} detail="Next-week probability" /></div></section>{!!stages.length && <section className="panel stage-panel" aria-live="polite"><div className="panel-head"><h3>Committee progress</h3><span>{stages[stages.length - 1]?.stage}</span></div><ol>{stages.map((stage, index) => <li className={index === stages.length - 1 ? "active" : ""} key={`${stage.stage}-${index}`}><b>{index + 1}</b><div><strong>{stage.stage}</strong><span>{stage.message}</span></div></li>)}</ol></section>}</div></details>
    </div>;
  }
