...
export function ResearchChat() {
  ...
  return <main className="app-shell">
    ...
    <header className="topbar"><div className="brand-lockup"><span className="omega" aria-hidden="true">Ω</span><div><p>OH MEGA CAPITAL</p><h1>Investment Command Center</h1></div></div><label className="profile-picker"><span>AI depth</span><select value={profile} onChange={(event) => setProfile(event.target.value as Profile)}><option value="flash">Flash · fastest</option><option value="think">Think · standard</option><option value="pro">Pro · deepest</option></select></label><div className="live-state" role="status"><i className={setupReady ? "ready" : ""} /><span>{setupReady ? "Systems operational" : "Setup required"}</span></div></header>
    <div className="app-grid"><aside className="sidebar"><nav aria-label="Primary navigation"><span className="nav-heading">Main</span>{primaryNavigation.map((item) => <button className={view === item.id ? "active" : ""} aria-current={view === item.id ? "page" : undefined} onClick={() => setView(item.id)} key={item.id}><i aria-hidden="true">{item.icon}</i><em>{item.id === "committee" ? "Home" : item.shortLabel}</em></button>)}<details className="advanced-nav" open={advancedNavigation.some((item) => item.id === view)}><summary>Advanced</summary>{advancedNavigation.map((item) => <button className={view === item.id ? "active" : ""} aria-current={view === item.id ? "page" : undefined} onClick={() => setView(item.id)} key={item.id}><i aria-hidden="true">{item.icon}</i><em>{item.shortLabel}</em>{item.id === "universe" && pendingCount > 0 && <b aria-label={`${pendingCount} pending`}>{pendingCount}</b>}</button>)}</details></nav><div className="profile-note"><span>Current AI depth</span><strong>{profiles[profile].label}</strong><small>{profiles[profile].description}</small></div><div className="system-check"><span><i className={status.openRouter ? "ok" : ""} />AI committee</span><span><i className={status.yahoo ? "ok" : ""} />Market data {stockProviderCount}/4</span><span><i className={status.persistence ? "ok" : ""} />Decision history</span><span><i className={status.scheduler ? "ok" : ""} />Weekly automation</span></div><p className="simulation-label">SIMULATED PORTFOLIO ONLY</p></aside>
