...
export async function calculateRiskDashboard() {
  ...
  type RunShape = {
    ...
    market: { mechanicalMode?: "Balanced" | "Attach" | "Lockdown"; features?: Array<{ ticker: string; return1w: number }> };
  ...
  const controlPeriods = predictions.slice(0, -1).map((run, index) => {
    ...
    const committeeStockWeight = modeWeights((run.final.mode ?? "Balanced") as "Balanced" | "Attach" | "Lockdown").stockPct / 100;
